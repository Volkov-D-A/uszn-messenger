...
- Все управляемые продуктом экраны следуют применимым критериям WCAG 2.2 A и AA: имеют логичный порядок фокуса, доступные названия элементов, видимый фокус, текстовые обозначения состояний и работоспособны при масштабировании до 200%.
...
│ Внимание             │ История сообщений                    │ Статус и роль         │
│ Объявления           │                                      │                      │
│ Закрепленные         │                                      │                      │
│ Личные               │                                      │                      │
│ Подразделения        ├──────────────────────────────────────┤                      │
...
- Левая панель: глобальный поиск, «Внимание», «Объявления», фильтры и активные чаты. Непрочитанное обозначается счетчиком и акцентом.
...
Глобальный поиск показывает только доступные пользователю данные и открывает результат в исходном контексте. Текст сообщений и объявлений ищется с учетом русской морфологии. Для имен, логинов и названий файлов используется совпадение по введенной подстроке без учета регистра. Интерфейс не обещает исправление опечаток, поиск по синонимам или содержимому файлов.
...
Для вложения старше 30 дней карточка с именем и размером остается в сообщении, но действие скачивания заменяется состоянием «Срок хранения истек».

Перед скачиванием доступного файла интерфейс предупреждает: «Файл получен от другого пользователя и не проверен USZN Messenger». Продукт не показывает ложный статус безопасности файла и не блокирует его по расширению.
...
Выбор `@tag` доступен только при наличии отдельного разрешения. Перед отправкой composer показывает число уведомляемых пользователей и требует подтверждения. Если состав тега изменился, появляется обновленное число и повторный запрос подтверждения; фиксированный максимум адресатов не применяется.
...
3. Необязательная критичность; срок ознакомления не задается.
...
## 8. Персональные уведомления и раздел «Внимание»

В меню каждого чата пользователь выбирает режим обычных уведомлений: «Все сообщения», «Только упоминания» или «Без уведомлений». Текущий режим обозначается в меню и, для отключенных уведомлений, пиктограммой рядом с названием чата. Интерфейс поясняет, что критичные сообщения продолжают уведомлять вне DND.

Раздел «Внимание» доступен из левой панели и показывает общий счетчик актуальных элементов. Внутри доступны фильтры «Все», «Упоминания» и «Требуют ознакомления». Карточка содержит источник, автора, время, краткий фрагмент и требуемое действие. Выбор карточки открывает исходное сообщение или объявление в контексте; после прочтения или подтверждения элемент исчезает согласно своему типу.
...
- создание пользователя и сброс пароля с однократным показом временного пароля, срока его действия и предупреждения о безопасной передаче;
- просмотр аудита и экспорт отфильтрованных событий в CSV при наличии отдельного разрешения;
- дерево подразделений;
- группы и теги;
- публичные, приватные и служебные каналы;
- роли, разрешения и области действия;
- лимит и общую квоту вложений;
- текущее заполнение хранилища вложений, предупреждение при 70% и состояние запрета загрузок при 95%;
...
5. Критичное сообщение: доставка, просмотр, «Ознакомлен» и список неподтвердивших без срока и напоминаний.
...
9. Переключение тем, сворачивание правой панели при уменьшении окна, масштабирование до 200% и прохождение ключевого сценария только с клавиатуры.
10. Разные режимы уведомлений для двух чатов и раздел «Внимание» с переходом к упоминанию и неподтвержденному критичному объекту.
